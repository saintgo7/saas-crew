...
function toDateInput(dateString: string) {
...
export default function EditReportPage({
  ...
  useEffect(() => {
    if (report && !initialized) {
      ...
      setPeriodStart(toDateInput(report.periodStart))
      setPeriodEnd(toDateInput(report.periodEnd))
      setSummary(report.summary || '')
      const s = report.sections as ReportSections
      setSections({
        learningSummary: s.learningSummary || '',
        achievements: s.achievements || '',
        courseDetails: s.courseDetails || '',
        issues: s.issues || '',
        nextPlans: s.nextPlans || '',
      })
      setInitialized(true)
    }
  }, [report, initialized])
  ...
  const handleSubmit = (status: 'DRAFT' | 'PUBLISHED') => {
    ...
    updateMutation.mutate(
      ...
      {
        onSuccess: () => router.push(`/admin/reports/${id}`),
      }
    )
  }
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={updateMutation.isPending || !title || !periodStart || !periodEnd}
